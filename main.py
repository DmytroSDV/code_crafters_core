...
def available_commands():
    command_list = ['contact-add', 'contact-find', 'contact-show-all', 'contact-phone-add', 'contact-email-add',
                    'contact-edit-phone', 'contact-edit-email', 'contact-edit-birthday', 'contact-remove', 'display-birthdays',
                    'note-add', 'note-find', 'note-show-all', 'note-edit', 'note-remove', 'tag-add', 'tag-find-sort', "file-sort",
                    'file-extension-add', 'file-extension-remove', 'quit', 'exit', 'q']
    command_explain = ['зберігає контакт з іменем, адресом, номером телефона, email та днем народження до книги контактів',
...
if __name__ == '__main__':
    ...
    if file_database.exists() and file_database.is_file():
        ...
        if not check_content:
            ...
            # в теорії для автоматизації можемо в цьому методі read_from_file видали input і передавати  тут file_name = 'database.bin' file_name
